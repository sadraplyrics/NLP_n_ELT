...
router_nums: Router = Router()


@router_nums.message(filters.Command(commands=["nums_facts"]))
...
@router_nums.message(lambda x: x.text and x.text.isdigit())
...
# ------------------------- WHEN PROMPTED -------------------------
@router_nums.message(filters.Text(text=["yes", "ok"], ignore_case=True))
...
@router_nums.message(filters.Text(text=["no", "nah"], ignore_case=True))
async def funny_nums_no(message: types.Message):
    if states["prompted"]:
        states["prompted"] = False
        states["nums_active"] = False
        await message.answer("That's sad")